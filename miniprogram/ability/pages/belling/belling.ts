// ability/pages/belling/belling.ts
...
Page({
  ...
  setEmail(e:any){
    this.setData({
      myEmail:e.detail.value
    })
  },
  setBellringEmail(){
    if(this.data.myEmail.length > 7){
      wx.request({
        method:'POST',
        url: 'http://localhost:5000/users/setMyEmail',
        data:{
    ...
    wx.showModal({
      ...
      success(res: any) {
        if (res.confirm) {
          wx.request({
            method: 'POST',
            url: 'http://localhost:5000/users/disBellringSkin',
            data: {
              token: wx.getStorageSync('token'),
  ...
  onLoad() {
    ...
    wx.request({
      method: 'POST',
      url: 'http://localhost:5000/users/getMyBellring',
      data: {
        token: wx.getStorageSync('token')
      },
